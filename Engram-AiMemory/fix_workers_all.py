...
# Since I keep messing up Python indentation with inline replaces,
# I will grab the file, find the injected `_job_update_decay` 
# and properly indent everything after it. Or better yet, just write out the clean methods.
...
start_idx = content.find("    async def _job_update_decay(self) -> None:")

if start_idx != -1:
    # Find the next proper function
    end_idx = content.find("    async def _job_consolidate(self) -> None:", start_idx)
...
"""
    ...
    content = content[:start_idx] + clean_decay + content[end_idx:]
