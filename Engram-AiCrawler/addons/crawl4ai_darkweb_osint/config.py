...
class LLMProviderConfig(BaseModel):
    ...
    model: str = Field(
        default="glm-5",
        description="Model name (e.g., glm-5, llama3, gpt-4, claude-3-opus)",
    )
...
class DiscoveryConfig(BaseModel):
    ...
    max_results_per_engine: int = Field(
        default=50, description="Max results per engine"
    )
    timeout_per_engine: int = Field(
        default=30, description="Timeout per engine request"
    )
    parallel_engines: int = Field(
        default=5, description="Number of engines to query in parallel"
    )
    dedup_enabled: bool = Field(default=True, description="Enable result deduplication")
    dedup_threshold: float = Field(
        default=0.85, description="Similarity threshold for dedup"
    )
...
class ExtractionConfig(BaseModel):
    ...
    max_content_length: int = Field(
        default=100000, description="Max content length to extract"
    )
...
class AnalysisConfig(BaseModel):
    ...
    include_raw_content: bool = Field(
        default=False, description="Include raw content in results"
    )
