...
async def close_cache():
    ...
    if _cache_client is not None:
        try:
            ...
            logger.debug("Error closing cache client: %s", exc)
    if _cache_pool is not None:
        try:
            await _cache_pool.disconnect()
        except Exception as exc:
            logger.debug("Error disconnecting cache pool: %s", exc)
    _cache_client = None
    _cache_pool = None
...
class CacheLayer:
    ...
    async def _try_negative_cache(self, key: str) -> bool:
        ...
        try:
            ...
            logger.debug("Negative cache check failed for %s: %s", key, exc)
        return False
    ...
    async def _wait_for_lock_result(self, key: str) -> dict | None:
        ...
        for _ in range(max_retries):
            ...
            try:
                ...
                logger.debug("Poll cache failed while waiting for lock on %s: %s", key, exc)
    ...
    async def _handle_compute_error(self, key: str, exc: Exception, neg_key: str) -> None:
        ...
        try:
            ...
            logger.debug("Failed to set negative cache for %s: %s", key, exc)
        ...
        try:
            ...
            if acquired_lock_key is not None:
                try:
                    ...
                    logger.debug("Failed to release lock %s: %s", acquired_lock_key, exc)
