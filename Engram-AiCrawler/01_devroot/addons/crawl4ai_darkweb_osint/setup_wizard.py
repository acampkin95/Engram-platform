...
class SetupWizard:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.config_path = config_path or Path.home() / ".crawl4ai" / "darkweb_config.json"
    ...
    async def run(self, non_interactive: bool = False) -> DarkWebConfig:
        ...
        print("-" * 40)
        self.state.tor_host = self._prompt_with_default("Tor proxy host", self.state.tor_host)
        self.state.tor_port = int(
            self._prompt_with_default("Tor SOCKS port", str(self.state.tor_port))
        )
        self.state.tor_control_port = int(
            self._prompt_with_default("Tor control port", str(self.state.tor_control_port))
        )
        ...
        if self.state.tor_valid:
            ...
            print(f"✗ Tor connection failed: {tor_result.get('error', 'unknown error')}")
        ...
        if provider_info["requires_api_key"]:
            self.state.llm_api_key = os.getenv(f"{self.state.llm_provider.upper()}_API_KEY")
        ...
        if self.state.llm_valid:
            ...
            print(f"✗ LLM connection failed: {llm_result.get('error', 'unknown error')}")
        ...
        save_choice = self._prompt_with_default("Save configuration? (Y/n)", "Y").lower()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--non-interactive", action="store_true", help="Use defaults and env vars")
    parser.add_argument("--check-tor", action="store_true", help="Check Tor connection only")
    parser.add_argument("--check-llm", action="store_true", help="Check LLM connection only")
