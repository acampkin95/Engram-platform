...
const getClientPaths = () => {
  ...
  if (osType === 'macOS') {
    return {
      ...
      cursor: path.join(home, 'Library/Application Support/Cursor/User/globalStorage/rooveterinaryinc.roo-cline/settings/cline_mcp_settings.json'),
      windsurf: path.join(home, '.codeium/windsurf/mcp_config.json'),
      vscode: path.join(home, 'Library/Application Support/Code/User/globalStorage/rooveterinaryinc.roo-cline/settings/cline_mcp_settings.json')
    };
  } else if (osType === 'Windows') {
    const appData = process.env.APPDATA || path.join(home, 'AppData', 'Roaming');
    return {
      claude: path.join(appData, 'Claude', 'claude_desktop_config.json'),
      cursor: path.join(appData, 'Cursor', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline', 'settings', 'cline_mcp_settings.json'),
      windsurf: path.join(home, '.codeium', 'windsurf', 'mcp_config.json'),
    ...
    return {
      ...
      cursor: path.join(config, 'Cursor', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline', 'settings', 'cline_mcp_settings.json'),
      windsurf: path.join(home, '.codeium', 'windsurf', 'mcp_config.json'),
      vscode: path.join(config, 'Code', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline', 'settings', 'cline_mcp_settings.json')
    };
  }
};
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      try {
        ...
        printWarning(`Could not parse existing config at ${configPath}. Creating new.`);
      }
    }
...
async function main() {
  ...
  const isMcpRoot = fs.existsSync(path.join(currentDir, 'package.json')) &&
                    JSON.parse(fs.readFileSync(path.join(currentDir, 'package.json'))).name === 'engram-mcp';
  ...
  if (!fs.existsSync(indexTsPath)) {
    ...
    if (answer.toLowerCase() !== 'y') {
      ...
      try {
        ...
        execSync(`cd ${mcpDir} && npm install && npm run build`, { stdio: 'inherit' });
  ...
  if (clientNames.length === 0) {
    ...
    printSuccess(`Detected ${clientNames.length} clients: ${clientNames.join(', ')}`);
  }
  ...
  const mcpName = await question('Enter MCP server name (default: engram-memory): ') || 'engram-memory';

  const weaviateUrl = await question('Weaviate URL (default: http://localhost:8080): ') || 'http://localhost:8080';
  const memoryApiUrl = await question('Memory API URL (default: http://localhost:8000): ') || 'http://localhost:8000';
  ...
  for (const [client, configPath] of Object.entries(detectedClients)) {
    const install = await question(`Install to ${client} at ${configPath}? (Y/n): `);
    if (install.toLowerCase() !== 'n') {
      if (installMCPToConfig(configPath, mcpName, defaultCommand, defaultArgs, env)) {
  ...
  if (manualInstall.toLowerCase() === 'y') {
    ...
    if (customPath) {
      if (installMCPToConfig(customPath, mcpName, defaultCommand, defaultArgs, env)) {
  ...
  printInfo('Configuring hooks for AI clients (Codex, OpenCode, Claude Code)...');

  const setupHooks = await question('Setup prompt hooks for supported CLI tools? (Y/n): ');
...
`;
      ...
      if (!updatedProfile) {
        printWarning('Could not find shell profile to update. Please add manually:');
  ...
  printInfo('Please restart your AI clients and terminal for the changes to take effect.');
