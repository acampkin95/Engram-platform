...
describe("createMCPServer", () => {
	...
	describe("ListToolsRequestSchema handler", () => {
		...
		it("tools have required properties", () => {
			for (const tool of ALL_TOOLS) {
				...
				assert.ok(
					tool.description,
					`tool should have description: ${tool.name}`,
				);
				assert.ok(
					tool.inputSchema,
					`tool should have inputSchema: ${tool.name}`,
				);
			}
		});
	});
	...
	describe("CallToolRequestSchema handler", () => {
		...
		it("rejects unknown tool name", () => {
			const unknownTool = ALL_TOOLS.find(
				(t) => t.name === "nonexistent_tool_xyz",
			);
			assert.strictEqual(
				unknownTool,
				undefined,
				"unknown tool should not exist",
			);
		});
		...
		it("tools have valid input schemas", () => {
			for (const tool of ALL_TOOLS) {
				...
				assert.ok(
					...
					`${tool.name} inputSchema should be object`,
				);
				assert.ok(
					schema.type === "object",
	...
	describe("ListResourcesRequestSchema handler", () => {
		it("returns static resources", () => {
			assert.ok(
				...
				"STATIC_RESOURCES should be an array",
			);
			assert.ok(
				STATIC_RESOURCES.length > 0,
				"STATIC_RESOURCES should not be empty",
			);
		});
		...
		it("resources have required properties", () => {
			for (const resource of STATIC_RESOURCES) {
				assert.ok(
					resource.uri,
					`resource should have uri: ${JSON.stringify(resource)}`,
				);
	...
	describe("ListResourceTemplatesRequestSchema handler", () => {
		it("returns resource templates", () => {
			assert.ok(
				Array.isArray(RESOURCE_TEMPLATES),
				"RESOURCE_TEMPLATES should be an array",
			);
			assert.ok(
				RESOURCE_TEMPLATES.length > 0,
				"RESOURCE_TEMPLATES should not be empty",
			);
		});
		...
		it("templates have required properties", () => {
			for (const template of RESOURCE_TEMPLATES) {
				assert.ok(
					...
					`template should have uriTemplate: ${JSON.stringify(template)}`,
				);
				assert.ok(
					template.name,
					`template should have name: ${template.uriTemplate}`,
				);
			}
		});
	});
	...
	describe("ReadResourceRequestSchema handler", () => {
		it("handles resource read requests", () => {
			for (const resource of STATIC_RESOURCES) {
				assert.ok(
					resource.uri,
					`resource should have readableURI: ${resource.uri}`,
				);
			}
		});
	...
	describe("ListPromptsRequestSchema handler", () => {
		...
		it("prompts have required properties", () => {
			for (const prompt of PROMPTS) {
				assert.ok(
					prompt.name,
					`prompt should have name: ${JSON.stringify(prompt)}`,
				);
				assert.ok(
					prompt.description,
					`prompt should have description: ${prompt.name}`,
				);
				assert.ok(
					prompt.arguments !== undefined,
					`prompt should have arguments: ${prompt.name}`,
				);
			}
		});
	});
			...
			for (const prompt of PROMPTS) {
				if (prompt.arguments && prompt.arguments.length > 0) {
					for (const arg of prompt.arguments) {
						assert.ok(
							arg.name,
							`prompt argument should have name: ${prompt.name}/${arg.name}`,
						);
					}
				}
			}
	...
	describe("error handling", () => {
		it("InternalServerError is constructable", () => {
			...
			assert.ok(
				...
				"InternalServerError should extend Error",
			);
	...
	describe("tool categorization", () => {
		it("categorizes tools by type", () => {
			const memoryTools = ALL_TOOLS.filter(
				(t) =>
			...
			const entityTools = ALL_TOOLS.filter(
				(t) =>
					t.name.startsWith("add_entity") ||
					t.name.startsWith("add_relation") ||
					t.name.startsWith("query_graph"),
			);
			const investigationTools = ALL_TOOLS.filter(
				(t) =>
					t.name.startsWith("create_matter") ||
					t.name.startsWith("ingest_") ||
					t.name.startsWith("search_matter"),
			);
		...
		it("all tools have descriptions", () => {
			for (const tool of ALL_TOOLS) {
				assert.ok(
					...
					`${tool.name} should have description`,
				);
			}
		});
	});
		...
		it("all prompts have descriptions", () => {
			for (const prompt of PROMPTS) {
				assert.ok(
					...
					`${prompt.name} should have description`,
				);
			}
		});
		...
		it("prompts define argument schemas", () => {
			for (const prompt of PROMPTS) {
				assert.ok(
					...
					`${prompt.name} should have arguments array`,
				);
			}
		});
	});
