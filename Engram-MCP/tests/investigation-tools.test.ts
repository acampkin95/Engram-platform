...
function createMockClient() {
	return {
		createMatter: mock.fn(
			async (data: { matter_id: string; title: string }) => ({
		...
		searchMatter: mock.fn(async () => ({
			results: [{ chunk_id: "c1", content: "result 1", score: 0.9 }],
			total: 1,
			query: "test",
		})),
...
describe("handleInvestigationTool", () => {
	...
	describe("create_matter", () => {
		it("creates matter with required fields", async () => {
			const result = await handleInvestigationTool(
				"create_matter",
				{
	...
	describe("search_matter", () => {
		...
		it("accepts limit at boundaries", async () => {
			// Test min boundary
			const result1 = await handleInvestigationTool(
				"search_matter",
				{
					matter_id: "CASE-001",
					query: "test",
			...
			// Test max boundary
			const result50 = await handleInvestigationTool(
				"search_matter",
				{
					matter_id: "CASE-001",
					query: "test",
